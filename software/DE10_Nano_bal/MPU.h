...
private:
	...
	uint8_t devAddr;
protected:
	alt_u32 m_ControllerAddr;
	bool WriteReg(uint32_t devAddr, alt_u8 RegIndex, alt_u8 RegValue);
	bool writeBits(uint32_t devAddr, alt_u8 RegIndex, alt_u8 BitIndex, bool bSet=1);
	bool writeBits(uint32_t devAddr, alt_u8 RegIndex, alt_u8 BitEndIndex, alt_u8 BitNum, alt_u8 Value);

	bool readBytes(uint32_t devAddr, alt_u8 RegIndex, alt_u16 Len, alt_u8 *pRegValue);
	bool readBits(uint32_t devAddr, alt_u8 RegIndex, alt_u8 BitEndIndex, alt_u8 BitNum, alt_u8 *Buffer);



public:
	MPU(alt_u32 ControllerAddr, alt_u8 DeviceAddr = 0xD0);
	virtual ~MPU();
	...
	void setSample_div(uint8_t div_rate);
	void setIntEnable(uint8_t enable);
	void setDLPF(uint8_t config);
